...
class PDFDestacados {
  ...
  final Jornada jornada;
   List<Player> jugadoresPorteros=new List();
   List<Player> jugadoresDefensas=new List();
   List<Player> jugadoresCentrocampistas=new List();
   List<Player> jugadoresDelanteros=new List();
  List<Player> jugadoresPorterosSUB=new List();
  List<Player> jugadoresDefensasSUB=new List();
  List<Player> jugadoresCentrocampistasSUB=new List();
  List<Player> jugadoresDelanterosSUB=new List();


  PDFDestacados({@required this.jugadores,@required this.categoria,@required this.jornada,
  });
...
//Add invoice footer
  ...
  Future header(PdfPage page, Size pageSize, String titulo) async {
    ...
    page.graphics.drawString("${categoria.categoria} - Jornada:${jornada.jornada}- ${jornada.fecha}",
  ...
  void drawGridDatosJugadoresPorteros(PdfPage page, PdfGrid grid, PdfLayoutResult result) async {
    ...
    result = grid.draw(
        page: page, bounds: Rect.fromLTWH(30, 130, 0, 0));
  }
  ...
  void drawGridDatosJugadoresDefensas(PdfPage page, PdfGrid grid, PdfLayoutResult result) async {
    ...
    //Draw the PDF grid and get the result.
    result = grid.draw(
        page: page, bounds: Rect.fromLTWH(210, 130, 0, 0));
  }
  ...
  void drawGridDatosJugadoresDelanteros(PdfPage page, PdfGrid grid, PdfLayoutResult result) async {
    ...
    //Draw the PDF grid and get the result.
    result = grid.draw(
        page: page, bounds: Rect.fromLTWH(590, 130, 0, 0));
  }
  ...
  void drawGridDatosJugadoresMedios(PdfPage page, PdfGrid grid, PdfLayoutResult result) async {
    ...
    //Draw the PDF grid and get the result.
    result = grid.draw(
        page: page, bounds: Rect.fromLTWH(400, 130, 0, 0));
  }
//Create PDF grid and return
  Future<PdfGrid> getGridJugadores(PdfPage page, PdfGrid grid, List<Player> jugadoresAux, int posicion) async{
    //Create a PDF grid
    ...
    double i=0;
    for (Player doc in jugadoresAux) {
      double altura=i*33;
      i++;
      //Add rows
      ...
      try{
        page.graphics.drawImage(
            ...
            Rect.fromLTWH(posicion.roundToDouble(), 134+altura, 22, 22));
      }catch(e){}
