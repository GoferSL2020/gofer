...
//insertion
  ...
  getJugadores(Equipo _equipo)  {
    List<Jugador> lista = new List();
    //print("getJugadores");
    String path2 =
        "temporadas/2021-2022/paises/${_equipo.pais}/categorias/${_equipo
        .categoria}/equipos/${_equipo.equipo}/jugadores";

    final DatabaseReference query =  FirebaseDatabase.instance.reference().child(
        path2);
    var dataSnapshot =   query.once().then((
         snapshot) {
      Map<dynamic, dynamic> values = snapshot.value;

      //print(values.toString());
      values.forEach((k, v) {
        Jugador jugador = Jugador.fromMap(v);
